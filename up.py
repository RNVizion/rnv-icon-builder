...
For rnv-icon-builder, derived against a fresh clone at the live head.
...
WHAT THIS REPOSITORY NEEDED.
    ...
    The only repository whose declaration matched what it tested -- >=3.10, and
a CI matrix that really did run 3.10, 3.11, 3.12 and 3.13. It paid for that
in ui/theme_manager.py, which carried a note explaining that the type alias
avoided PEP 695 "so the module imports on Python 3.10 and 3.11".

    pyproject.toml               >=3.10 -> >=3.13, PyQt6 capped below 7.0,
                                 four classifiers -> one
    requirements.txt             PyQt6>=6.6.0 -> PyQt6>=6.6.0,<7.0
    tests/requirements-dev.txt   pytest-timeout>=2.4,<3.0
    both workflows               a four-version matrix -> ["3.13"]
    ui/theme_manager.py          type ThemeDict = dict[str, str]
    the marker                   tests/test_brand_contrast.py

THAT NOTE WAS LOAD-BEARING and this script is what makes it false. Eight CI
jobs become two. Measured on a real 3.13 before writing this: 655 + 471.

RUN ORDER, AND A SCRIPT ALREADY SITTING IN THIS REPOSITORY. Every one of
the five has a committed up.py at its head; in the other four it is the
no-vacuous-tests script that has already been run, and here it is the one
that has NOT -- tests/test_no_vacuous_tests.py is not in this checkout and
tests/conftest.py carries no stamp from it.

So copying this script over up.py would overwrite the only copy of it you
have. Run the one that is already there first, then replace it with this.

If they do land the other way round, nothing breaks: both stamp
tests/conftest.py, each anchors on text the other leaves alone, and the two
orders were run end to end here and produce byte-identical trees apart from
which comment block sits first. Tested both ways rather than reasoned
about.
...
SUITES = [("\"pytest tests/\"",
           [sys.executable, "-m", "pytest", "tests/", "-q",
            "-p", "no:cacheprovider"]),
          ("\"the LOCKED file\"",
           [sys.executable, "-m", "pytest", "test_rnv_icon_builder.py", "-q",
